...
export const getMyLinksHandler = async (request, reply) => {
    ...
    try {
        ...
        const withBuffered = await Promise.all(links.map(async (link) => ({
            ...link,
...
export const updateLinkHandler = async (request, reply) => {
    try {
...
// 3. Delete a link
export const deleteLinkHandler = async (request, reply) => {
    try {
        const { id } = request.params;
        const jwtUser = request.user;
        const link = await prisma.link.findUnique({ where: { id } });
...
export const getLinkQRHandler = async (request, reply) => {
    try {
        const { id } = request.params;
        const jwtUser = request.user;
...
export const getLinkStatsHandler = async (request, reply) => {
    try {
        const { id } = request.params;
        const jwtUser = request.user;
        const link = await prisma.link.findUnique({ where: { id } });
