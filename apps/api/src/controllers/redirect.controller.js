...
export const redirectHandler = async (request, reply) => {
...
export const unlockHandler = async (request, reply) => {
    const { slug } = request.params;
    ...
    if (!link.isActive)
        return reply.status(403).send({ error: 'Link is disabled' });
    if (!link.password)
        return reply.status(400).send({ error: 'Link is not password protected' });
